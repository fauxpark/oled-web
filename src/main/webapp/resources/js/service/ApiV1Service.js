 ...
 */
oled.service('ApiV1Service', ['$http', 'AlertService', function($http, AlertService) {
  return {
    /**
    ...
    get: function(url, callback) {
      $http.get(path + 'api/v1/' + url).then(function(response) {
        if(response.data.ok) {
          if(callback) {
    ...
    post: function(url, data, callback) {
      $http.post(path + 'api/v1/' + url, data).then(function(response) {
        if(response.data.ok) {
          if(callback) {
            callback(response.data);
          }
    ...
    postImage: function(url, data, callback) {
      $http.post(path + 'api/v1/' + url, data, {
